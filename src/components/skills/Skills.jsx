...
const Skills = () => {
  ...
  const scrollToSection = (id, delay = 800) => {
    setTimeout(
  ...
  const executeCommand = (cmd) => {
    ...
    const output = commands[command]
      ? commands[command]()
      : [`> COMMAND NOT RECOGNIZED: ${cmd}`, ...commands["help"]()];
  ...
  useEffect(() => setTerminalOutput([...initialOutput]), []);
  useEffect(
  ...
  const SkillCard = ({ name, icon, color }) => (
    <div className="flex flex-col items-center p-4 rounded-xl bg-white dark:bg-gray-700/50 border border-gray-100 dark:border-gray-700/50 shadow-sm transition-all duration-300 hover:shadow-md hover:border-cyan-300 dark:hover:border-cyan-700">
      <div className={`text-3xl mb-2 ${color ? `skill-icon-${color}` : ""}`}>{getIconComponent(icon)}</div>
  ...
  const CategoryCard = ({ category, skills }) => (
    <div className="rounded-xl overflow-hidden shadow-lg transform transition-all duration-300 hover:scale-[1.02] hover:shadow-xl bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 border border-gray-200 dark:border-gray-700">
  ...
  return (
    <section
      id="skills"
      className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto bg-white dark:bg-gray-900 text-gray-900 dark:text-green-400 font-mono scroll-mt-20"
    >
      ...
      <div className="mt-6 text-center text-gray-600 dark:text-gray-400 text-sm">
        <p>
          ...
          USER:{" "}
          <span className="text-cyan-700 dark:text-cyan-400">DEVELOPER</span>
        </p>
        <p className="mt-2">
          TIP: Try typing{" "}
          <span className="text-cyan-700 dark:text-cyan-400">show skills</span>{" "}
          to explore my technical expertise
        </p>
      </div>
