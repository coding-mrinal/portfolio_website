...
const Education = () => {
  const educationTimeline = [
    ...
    },
    {
      ...
      achievements: [
        "School topper",
        "Led My House To Be The Inter-house champion",
      ]
    }
  ];

  const TimelineConnector = () => (
    <div className="absolute left-6 sm:left-8 md:left-1/2 transform md:-translate-x-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-purple-300 via-indigo-300 to-blue-300 dark:from-purple-600 dark:via-indigo-600 dark:to-blue-600" />
  );

  return (
    <section id="education" className="py-12 sm:py-16 md:py-20 bg-gradient-to-br from-gray-50 via-white to-gray-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 overflow-hidden">
      {/* Header */}
      <motion.div
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            ...
            >
              ...
              <div
                className={`absolute left-6 sm:left-8 md:left-1/2 transform md:-translate-x-1/2 z-20 
                  w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-full bg-gradient-to-r ${item.color} 
              ...
              {/* Content Card */}
              <div
                className={`ml-16 sm:ml-20 md:ml-0 md:w-1/2 ${
                  index % 2 === 0 ? 'md:pr-12' : 'md:pl-12'
                }`}
              >
                <div
                  className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-lg 
                ...
                >
                  ...
                  <div className="p-4 sm:p-6 md:p-8">
                    {/* Year Badge */}
                    <div
                      className="inline-flex items-center gap-2 px-2.5 sm:px-3 py-1 rounded-full 
                        bg-gradient-to-r from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-600 
                        text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-200 mb-3 sm:mb-4"
                    >
                      <FaCalendarAlt className="text-xs" />
                      <span>{item.year}</span>
                      <span className="ml-2 text-xs bg-green-500 text-white px-1.5 sm:px-2 py-0.5 rounded-full">
                        {item.milestone}
                      </span>
                    </div>

                    {/* Title & Institution */}
                    <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-800 dark:text-white mb-2 leading-tight">
                      {item.title}
                    </h3>
                    ...
                    {/* Fun Fact */}
                    <div
                      className="mb-4 sm:mb-6 p-3 sm:p-4 bg-gradient-to-r from-yellow-50 to-orange-50 
                        dark:from-yellow-900/20 dark:to-orange-900/20 rounded-lg sm:rounded-xl 
                        border border-yellow-200 dark:border-yellow-800"
                    >
                      <div className="flex items-start gap-2 sm:gap-3">
                        <FaLightbulb className="text-yellow-500 mt-0.5 sm:mt-1 flex-shrink-0 text-sm sm:text-base" />
                        <p className="text-xs sm:text-sm md:text-base text-gray-700 dark:text-gray-300 italic leading-relaxed">
                          "{item.funFact}"
                        </p>
                      </div>
                    </div>
                  ...
                  <div className={`absolute -bottom-10 -right-10 w-24 sm:w-32 h-24 sm:h-32 rounded-full 
                    bg-gradient-to-r ${item.color} opacity-10 blur-2xl`} />
                </div>
              </div>
            </motion.div>
          ))}
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="text-center mt-12 sm:mt-16 md:mt-20 relative z-10"
        >
